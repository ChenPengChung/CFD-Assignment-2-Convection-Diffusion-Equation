...
void initial(vector<vector<double> >& a, vector<double>& b , vector<double>& x) {
    ...
    a[1][1] = 14.0 * Gamma + dx * (7.0/8.0) * u + dy * (7.0/8.0) * v ;
    a[1][2] = (-5.0/3.0) * Gamma + dy * (3.0/8.0) * u;  // T_{1,0}
    a[1][NX+1] = (-5.0/3.0) * Gamma + dx * (3.0/8.0) * v;  // T_{0,1}
    b[1] = T_l * ((16.0/3.0) * Gamma + dy * (5.0/4.0) * u ) + T_b * ((16.0/3.0)*Gamma + dx * (4.0/5.0) * v);  // 左邊界=1.0

    //第二個點T_{1,0} -> a[2][2] (下邊界第二排)
    a[2][2] = 9.0 * Gamma + dy * (3.0/8.0) * u + dx * (7.0/8.0) * v ;
    a[2][1] = -1.0 * Gamma - dy * u ;  // T_{0,0}
    a[2][3] = -1.0 * Gamma + dy * (3.0/8.0) * u ;  // T_{2,0}
    a[2][NX+2] = (-5.0/3.0) * Gamma + dx * (3.0/8.0) * v ;  // T_{1,1}
    b[2] = -dy * (1.0/4.0) * u * T_l + ((16.0/3.0) * Gamma + dx * (10.0/8.0) * u) * T_b ;  // 下邊界=0.0

    //第三個點T_{NX-1,0} -> a[NX][NX] (右下角)
    a[NX][NX] = 8.0 * Gamma + dy * (6.0/8.0) * u + dx * (7.0/8.0) * v ;
    a[NX][NX-1] = -Gamma + dy * (-7.0/8.0) * u ;  // T_{NX-2,0}
    a[NX][NX-2] = dy * (1.0/8.0) * u ;  // T_{NX-3,0}
    a[NX][2*NX] = (-5.0/3.0) * Gamma + dx * (3.0/8.0) * v ;  // T_{NX-1,1}
    b[NX] =((10.0/8.0)*dx+Gamma*(16.0/3.0))* T_b ;  // 右邊界和下邊界都=0.0

    //第四個點T_{0,1} -> a[NX+1][NX+1] (左邊界第二排)
    a[NX+1][NX+1] = 9.0 * Gamma + dy * (7.0/8.0) * u  + dx * (3.0/8.0) * v ;
    a[NX+1][NX+2] = (-5.0/3.0) * Gamma + dy * (3.0/8.0) * u ;  // T_{1,1}
    a[NX+1][1] = -1.0 * Gamma - dx * v ;  // T_{0,0}
    a[NX+1][2*NX+1] = -1.0 * Gamma + dx * (3.0/8.0) * v ;  // T_{0,2}
    b[NX+1] = ((16.0/3.0) * Gamma + dy * (10.0/8.0) * u) * T_l + (-dx * (2.0/8.0) * v)*T_b ;
    ...
    a[2*NX][2*NX] = 3.0 * Gamma + dy * (6.0/8.0) * u  + dx * (3.0/8.0) * v ;
    a[2*NX][2*NX-1] = -Gamma + dy * (-7.0/8.0) * u;  // T_{NX-2,1}
    ...
    a[n-NX+1][n-NX+1] = 8*Gamma + dy*(7.0/8.0) + dx*(6.0/8.0);
    a[n-NX+1][n-NX+2] = (-5.0/3.0)*Gamma+(3.0/8.0)*dy ;  // T_{1,NY-1}
    a[n-NX+1][n-2*NX+1] = -Gamma+(-7.0/8.0)*dx;  // T_{0,NY-2}
    a[n-NX+1][n-3*NX+1] = (1.0/8.0)*dx;  // T_{0,NY-3}
    b[n-NX+1] = T_l*((5.0/4.0)*dy+(16.0/3.0)*Gamma);  // 左邊界和上邊界都=1.0

    //第八個點T_{1,NY-1} -> a[n-NX+2][n-NX+2] (上邊界第二排)
    a[n-NX+2][n-NX+2] = 3*Gamma + (3.0/8.0)*dy + (6.0/8.0)*dx ;
    a[n-NX+2][n-NX+1] = -Gamma + (-1.0)*dy ;
    a[n-NX+2][n-NX+3] = -Gamma + (3.0/8.0)*dy ;
    a[n-NX+2][n-2*NX+2] = -Gamma + (-7.0/8.0)*dx;
    a[n-NX+2][n-3*NX+2] = dx*(1.0/8.0);
    b[n-NX+2] = T_l * (-1.0/4.0)*dy ;

    //第九個點T_{NX-1,NY-1} -> a[n][n] (右上角)
    a[n][n] = 2*Gamma+(6.0/8.0)*dy+(6.0/8.0)*dx ;
    a[n][n-1] = -Gamma +(-7.0/8.0)*dy;  // T_{NX-2,NY-1}
    a[n][n-2] = (1.0/8.0)*dy;  // T_{NX-3,NY-1}
    a[n][n-NX] =-Gamma+(-7.0/8.0)*dx;  // T_{NX-1,NY-2}
    ...
    for(int i = 3; i <= NX-1; i++) {
        a[i][i] = 9.0 * Gamma + dy * (3.0/8.0) * u + dx * (7.0/8.0) * v;
        a[i][i-1] = -1.0 * Gamma - dy * (7.0/8.0) * u;
        a[i][i+1] = -1.0 * Gamma + dy * (3.0/8.0) * u;
        a[i][i-2] = dy * (1.0/8.0) * u;
        a[i][i+NX] = (-5.0/3.0) * Gamma + dx * (3.0/8.0) * v;
        b[i] = (16.0/3.0) * Gamma * T_b + dx * (10.0/8.0) * v * T_b ;  // 下邊界=0.0
    }
    
    // 上邊界 (除角點外)
    for(int i = n-NX+3; i <= n-1 ; i++) {
        a[i][i] = 3*Gamma + (3.0/8.0)*dy + (6.0/8.0)*dx;
        a[i][i-1] = -Gamma + dy*(-7.0/8.0);//西計算點
        a[i][i+1] = -Gamma + dy*(3.0/8.0) ;//東計算點
        a[i][i-2] = (1.0/8.0)*dy ;//西西計算點
        a[i][i-NX] = -Gamma + dx*(-7.0/8.0);//南計算點
    ...
    for(int i = 3 ; i <= NX-1 ; i++) {
        int idx = NX * (i-1) + 1;
        a[idx][idx] = 9.0 * Gamma + dy * (7.0/8.0) * u  + dx * (3.0/8.0) * v ;
        a[idx][idx+1] = (-5.0/3.0) * Gamma + dy * (3.0/8.0) * u ;
        a[idx][idx-NX] = -1.0 * Gamma - dx * (7.0/8.0) * v ;
        a[idx][idx+NX] = -1.0 * Gamma + dx * (3.0/8.0) * v ;
        a[idx][idx-2*NX] = dx * (1.0/8.0) * v ;
        b[idx] = (16.0/3.0) * Gamma * T_l + dy * (10.0/8.0) * u * T_l ;  // 左邊界=1.0
    }
    
    // 右邊界 (除角點外)
    for(int i = 3; i <= NX-1; i++) {
        int idx = NX * i;
        a[idx][idx] = 3.0 * Gamma + dy * (6.0/8.0) * u + dx * (3.0/8.0) * v ;
        a[idx][idx-1] =  -Gamma + dy * (-7.0/8.0) * u;
        a[idx][idx-2] = dy * (1.0/8.0) * u ;
        a[idx][idx-NX] = -Gamma + dx * (-7.0/8.0) * v ;
        a[idx][idx+NX] = -1.0 * Gamma + dx * (3.0/8.0) * v ;
        a[idx][idx-2*NX] = dx * (1.0/8.0) * v ;
        b[idx] = 0.0 ;  // 右邊界=0.0
    }
...
void output(int m) {
    ...
    ostringstream name;
    name << "QUIUCK_1JC(QUICK)0925_Pe = " << Pelect <<","<<"Grid "<< nodes_i << "x" << nodes_j << ","<< setfill('0') << setw(6) << m << ".vtk";
    ofstream out(name.str().c_str());
    
    // VTK 文件頭：使用 STRUCTURED_GRID
    out << "# vtk DataFile Version 3.0\n";
    out << "QUIUCK_1JC(QUICK)0925\n";
    out << "ASCII\n";
