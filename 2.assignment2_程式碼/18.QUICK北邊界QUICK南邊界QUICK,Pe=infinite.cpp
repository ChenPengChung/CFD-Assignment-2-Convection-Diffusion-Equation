...
void Gamma0(vector<vector<double> >& a, vector<double>& b , vector<double>& x) {
    ...
    //第九個角點 (NX, NX)
    int p9 = (NX-1)*NX + NX;
    a[p9][p9] = dx*(3.0/8.0) + dx*(3.0/8.0) ;         //本點（a2原有 + a2從北側移來）
    a[p9][p9-1] = dx*(-7.0/8.0) ;           //西側計算點
    a[p9][p9-2] = dx*(1.0/8.0) ; 
    a[p9][p9-NX] = dx*(-7.0/8.0) ;          //南側計算點
    a[p9][p9-2*NX] = dx*(1.0/8.0) ;         //南南側計算點
    b[p9] = 0.0 ;
    ...
    for(int i = 3; i <= NX-1; i++) {
        int index = (NX-1)*NX + i;
        a[index][index] = a2 + a3 ;       //本點（注意：無上側項）
        a[index][index+1] = a2;     //東側計算點
        a[index][index-1] = -a4;    //西側計算點
        a[index][index-2] = a1;     //西西側計算點
        a[index][index-NX] = -a4 ;
...
void output(int m) {
    ...
    ostringstream name;
    name << "QUICK北邊界QUICK南邊界QUICK,Pe=infinite,Grid "<< nodes_i << "x" << nodes_j << ","<< setfill('0') << setw(6) << m << ".vtk";
    ofstream out(name.str().c_str());
    
    // VTK 文件頭：使用 STRUCTURED_GRID
    out << "# vtk DataFile Version 3.0\n";
    out << "QUICK北邊界QUICK南邊界QUICK,Pe=infinite,Grid "<< nodes_i << "x" << nodes_j << ","<< setfill('0') << setw(6) << m << ".vtk\n";
    out << "ASCII\n";
