...
void initial(vector<vector<double> >& a, vector<double>& b , vector<double>& x) {
    ...
    a[NX+1][NX+2] = (-1) * Gamma + dy * (3.0/8.0) * u ;  // T_{1,1}
    ...
    a[n-NX+1][n-NX+1] = 4*Gamma + dy*(7.0/8.0) + dx*(6.0/8.0);
    ...
    for(int i = 3 ; i <= NX-1 ; i++) {
        int idx = NX * (i-1) + 1;
        a[idx][idx] = 5 * Gamma + dy * (7.0/8.0) * u  + dx * (3.0/8.0) * v ;
        a[idx][idx+1] = -1 * Gamma + dy * (3.0/8.0) * u ;
        a[idx][idx-NX] = -1.0 * Gamma - dx * (7.0/8.0) * v ;
        a[idx][idx+NX] = -1.0 * Gamma + dx * (3.0/8.0) * v ;
        a[idx][idx-2*NX] = dx * (1.0/8.0) * v ;
        b[idx] = 2 * Gamma * T_l + dy * (10.0/8.0) * u * T_l ;  // 左邊界=1.0
    }
...
void output(int m) {
    ...
    ostringstream name;
    name << "QUICK scheme_Pe = " << Pelect <<","<<"Grid "<< nodes_i << "x" << nodes_j << ","<< setfill('0') << setw(6) << m << ".vtk";
    ofstream out(name.str().c_str());
    
    // VTK 文件頭：使用 STRUCTURED_GRID
    out << "# vtk DataFile Version 3.0\n";
    out << "QUICK scheme_Pe = " << Pelect <<","<<"Grid "<< nodes_i << "x" << nodes_j << ","<< setfill('0') << setw(6) << m << ".vtk\n";
    out << "ASCII\n";
