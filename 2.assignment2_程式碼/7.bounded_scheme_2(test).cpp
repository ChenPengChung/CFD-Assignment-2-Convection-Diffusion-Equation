...
void output(int m) {
    ...
    ostringstream name;
    name << "bounded_scheme_2_Pe = " << Pelect <<","<<"Grid "<< nodes_i << "x" << nodes_j << ","<< setfill('0') << setw(6) << m << ".vtk";
    ofstream out(name.str().c_str());
    
    // VTK 文件頭：使用 STRUCTURED_GRID
    out << "# vtk DataFile Version 3.0\n";
    out << "bounded_scheme_2\n";
    out << "ASCII\n";
...
int main() {
    ...
    coefficient(a,b,n) ;//根據初始溫度進行賦值係數 
   for(G = 0; G < max_G; G++) {
        if(G % 1000== 0) {
            cout << "迭代次數 = " << G << endl;
            if(G > 0) {
