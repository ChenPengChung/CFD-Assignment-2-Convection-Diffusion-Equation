...
void Gamma0(vector<double>& x) { //因為係數式時變的,每一次迭代都必須要做.....
    ...
    double x_new2 =  ((-dx*T_right + dx*T_bottom) + x[index2-1]*dx*(1-(MUSCL_w(NX,1))/2.0) - x[index2+NX]*dx*(MUSCL_n(NX,1))/2.0 )/((-dx*(MUSCL_w(NX,1))/2.0) + dx*(1-(MUSCL_n(NX,1))/2.0));
    ...
    double x_new3 =  (dx*T_left - x[index3+1]*dx*(MUSCL_e(1,NX))/2.0 + x[index3-NX]*(dx*MUSCL_s(1,NX)/2.0) )/(dx*(1-(MUSCL_e(1,NX)/2.0)) + dx*(1-MUSCL_s(1,NX)/2.0)) ;
    ...
    double x_new4 =  ((-dx*T_right) + x[index4-1]*(dx*(1-MUSCL_w(NX,NX)/2.0)) + x[index4-NX]*(dx*(1-MUSCL_s(NX,NX)/2.0)))/(dx*(-MUSCL_w(NX,NX)/2.0) + dx*(1-MUSCL_s(NX,NX)/2.0)) ;
    ...
    for(int j = 2; j <= NX-1; j++) {
        int index = (j-1)*NX + NX;
        x_old[index] = x[index] ;
        double x_new = (-dx*T_right +x[index-1]*(dx*(1-MUSCL_w(NX,j)/2.0)) -x[index+NX]*dx*MUSCL_n(NX,j)/2.0 +x[index-NX]*(dx*(1-MUSCL_s(NX,j)/2.0)) )/(dx*(-MUSCL_w(NX,j)/2.0)+dx*(1-MUSCL_n(NX,j)/2.0-MUSCL_s(NX,j)/2.0)) ;
        x[index] = x_old[index] + lamda * (x_new - x_old[index]);
        T[NX][j] = x[index] ;
        T[NX+1][j] = 0.0 ;
    }
